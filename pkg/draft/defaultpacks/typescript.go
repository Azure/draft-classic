...
const typescriptValues = `# Default values for node.
...
const typescriptDetect = `#!/usr/bin/env bash

BUILD_DIR=$1

# Exit early if app is clearly not Typescript.
...
// TypescriptFiles returns all of the files needed for the python default pack
// Paths are relative to the pack root
func TypescriptFiles() []*pack.File {
	return []*pack.File{
		{
			// values.yaml
