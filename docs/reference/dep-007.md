...
## Using the `auto-connect` subcommand from `draft up`
...
> If you are deploying to not default environment, you need to specify the environment in `auto-connect` flag too. Example: `draft up --environment=staging auto-connect --environment=staging`.

## Using the `auto-connect` field in `draft.toml`
If you regularly want to automatically connect after upgrading the application, you can add a field in `draft.toml` that will have the same effect as using `auto-connect` subcommand, and keeping the port mapping config:
