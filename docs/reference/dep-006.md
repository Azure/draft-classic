...
- `registry`: the name of the Docker registry to publish the image to. This can be overridden globally by setting the `registry` field with `draft config set registry <name>`.
